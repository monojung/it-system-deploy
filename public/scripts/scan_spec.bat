@echo off
chcp 65001 >nul
title Thung Hua Chang Hospital - Client Hardware Audit Utility
powershell -NoProfile -ExecutionPolicy Bypass -Command "$content = [System.IO.File]::ReadAllText('%~f0', [System.Text.Encoding]::UTF8); $psCode = $content -replace '(?s)^.*?:::POWERSHELL_START:::\r?\n', ''; [ScriptBlock]::Create($psCode).Invoke()"
echo.
echo ==========================================================================
echo   เสร็จสิ้น! ข้อมูลสเปคถูกคัดลอกลงคลิปบอร์ดแล้ว
echo   กรุณาสลับกลับไปที่หน้าเว็บเบราว์เซอร์ แล้วกดปุ่ม "วางสเปคจากคลิปบอร์ด"
echo ==========================================================================
echo.
