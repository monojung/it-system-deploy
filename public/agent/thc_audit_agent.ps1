...
param(
    [string]$ServerUrl = "http://192.168.2.89/it-system/api/hardware-audit/submit"
)
...
try {
    ...
    $req = [System.Net.HttpWebRequest]::Create($ServerUrl)
    $req.Method = "POST"
    ...
    Write-Host "  [NOTICE] Direct HTTP transmission error: $_" -ForegroundColor DarkYellow
    Write-Host "  Trying fallback via Invoke-RestMethod..." -ForegroundColor Gray
    try {
        $respObj = Invoke-RestMethod -Uri $ServerUrl -Method Post -Body $jsonBody -ContentType 'application/json; charset=utf-8' -TimeoutSec 10
        Write-Host '  [SUCCESS] Telemetry sent via fallback!' -ForegroundColor Green
        Write-Host '  => สถานะ: บันทึกข้อมูลเข้าสู่ระบบเรียบร้อย (รอแอดมินกดตรวจสอบและอนุมัติ)' -ForegroundColor Yellow
        $transmitted = $true
    } catch {
        Write-Host "  [WARNING] Could not connect to IT server at $ServerUrl" -ForegroundColor Red
        Write-Host "  Please check network connection or verify IT server IP/URL." -ForegroundColor White
    }
}
