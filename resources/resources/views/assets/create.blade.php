...
@section('title', 'เพิ่มครุภัณฑ์ใหม่')
@section('page-title', 'เพิ่มครุภัณฑ์คอมพิวเตอร์และอุปกรณ์ IT')
@section('breadcrumb', 'ระบบพัสดุและคลัง / ทะเบียนครุภัณฑ์ IT / เพิ่มรายการใหม่')

@section('topbar-actions')
@if(auth()->user()->isAdmin())
<button type="button" class="topbar-btn" onclick="openImportModal()" title="นำเข้าข้อมูลครุภัณฑ์หลายรายการจากไฟล์ CSV">
    <i class="bi bi-file-earmark-arrow-up text-primary"></i>
    <span>นำเข้าไฟล์ CSV</span>
</button>
@endif
<a href="{{ route('assets.index') }}" class="topbar-btn" title="กลับไปหน้ารายการครุภัณฑ์">
...
<div style="max-width: 960px; margin: 0 auto; padding-bottom: 60px;">
    ...
    {{-- Admin Quick Import Callout Banner --}}
    @if(auth()->user()->isAdmin())
    <div style="background: linear-gradient(135deg, #f0fdfa 0%, #e0f2fe 100%); border: 1px solid #99f6e4; border-radius: 14px; padding: 16px 20px; margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 14px; box-shadow: 0 4px 12px rgba(13, 148, 136, 0.08);">
        <div style="display: flex; align-items: center; gap: 14px;">
            <div style="width: 44px; height: 44px; border-radius: 12px; background: #0d9488; color: #ffffff; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0;">
                <i class="bi bi-file-earmark-spreadsheet-fill"></i>
            </div>
            <div>
                <div style="font-size: 14.5px; font-weight: 700; color: #0f766e;">
                    มีข้อมูลครุภัณฑ์จำนวนมาก? นำเข้าได้ทันทีผ่านไฟล์ CSV
                </div>
                <div style="font-size: 12.5px; color: #334155; margin-top: 2px;">
                    ดาวน์โหลดเทมเพลตตัวอย่างที่มีสเปกเครื่อง (PC DDR5, Notebook DDR4, All-in-One, Printer) แล้วอัปโหลดเพื่อบันทึกเป็นชุด
                </div>
            </div>
        </div>
        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
            <a href="{{ route('assets.import.template') }}" class="btn btn-sm btn-light border" style="font-weight: 600; display: inline-flex; align-items: center; gap: 6px; font-size: 12.5px;" download>
                <i class="bi bi-download text-primary"></i> ดาวน์โหลดเทมเพลต CSV
            </a>
            <button type="button" onclick="openImportModal()" class="btn btn-sm btn-primary" style="font-weight: 600; display: inline-flex; align-items: center; gap: 6px; font-size: 12.5px;">
                <i class="bi bi-cloud-arrow-up-fill"></i> นำเข้าไฟล์ CSV
            </button>
        </div>
    </div>
    @endif
    ...
    <div style="margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
        <div>
            <h2 style="font-size: 20px; font-weight: 700; color: #0f172a; margin: 0 0 4px 0;">
                <i class="bi bi-plus-circle-fill text-primary me-1"></i> ลงทะเบียนครุภัณฑ์ใหม่ (Asset Registration)
            </h2>
            <p style="font-size: 13.5px; color: #64748b; margin: 0;">
                กรอกข้อมูลครุภัณฑ์และคุณลักษณะเฉพาะทางฮาร์ดแวร์เพื่อบันทึกลงสู่ระบบคลัง IT
            </p>
        </div>
        <a href="{{ route('assets.index') }}" class="btn btn-secondary btn-sm" style="display: inline-flex; align-items: center; gap: 6px;">
    ...
    <form action="{{ route('assets.store') }}" method="POST" enctype="multipart/form-data" id="createAssetForm">
                ...
                <div class="form-row">
                    <div class="form-group" style="flex: 1.2;">
                        ...
                        <input type="text" id="asset_code" name="asset_code" class="form-control"
                               value="{{ old('asset_code') }}" placeholder="เช่น 7440-001-0001/67" required autofocus>
                        <span class="form-text">รหัสหมายเลขครุภัณฑ์ตามทะเบียนพัสดุ รพ.ทุ่งหัวช้าง</span>
                    </div>
                ...
                <div style="background: linear-gradient(135deg, #f8fafc 0%, #f0fdfa 100%); border: 1.5px solid #99f6e4; border-radius: 12px; padding: 16px; margin-bottom: 20px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; flex-wrap: wrap; gap: 8px;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <div>
                                <strong style="font-size: 13.5px; color: #0f766e;">เลือกชื่อรายการตามเกณฑ์ราคากลางและคุณลักษณะพื้นฐานฯ</strong>
                    ...
                    <div style="margin-bottom: 10px;">
                        <select id="ict_standard_select" class="form-select" onchange="handleIctStandardSelect(this.value)" style="border-color: #5eead4; background-color: #ffffff; font-size: 13.5px; font-weight: 500;">
                            <option value="">-- คลิกเพื่อเลือกชื่อตามเกณฑ์ราคากลาง ICT (มีสเปกและราคากลางแนะนำ) --</option>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group" style="flex: 1.2;">
                        ...
                        <input type="text" id="location_detail" name="location_detail" class="form-control"
                               value="{{ old('location_detail') }}" placeholder="เช่น เคาน์เตอร์พยาบาล, โต๊ะทำงานชั้น 2">
                    ...
                    <div class="form-group" style="flex: 1;">
                        ...
                        <input type="text" id="custodian_name" name="custodian_name" class="form-control"
                               value="{{ old('custodian_name') }}" placeholder="เช่น พว.สุดาพร ใจดี">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="date" id="purchase_date" name="purchase_date" class="form-control"
                               value="{{ old('purchase_date') }}" onchange="handlePurchaseDateChange(this.value)">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div style="position: relative;">
                            <input type="number" step="0.01" id="price" name="price" class="form-control"
                                   value="{{ old('price') }}" placeholder="เช่น 24500.00" style="padding-right: 42px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="warranty_expire_date" name="warranty_expire_date" class="form-control"
                               value="{{ old('warranty_expire_date') }}">
                        <div style="display: flex; gap: 6px; margin-top: 6px; flex-wrap: wrap; align-items: center;">
            ...
            <div class="form-section-body">
                <div class="form-row">
                    <div class="form-group" style="flex: 1;">
                        ...
                        <div class="image-dropzone" onclick="document.getElementById('image').click()">
                            ...
                            <div id="imageUploadPrompt">
                                ...
                                <div style="font-size: 13px; font-weight: 600; color: #1e293b; margin-top: 4px;">คลิกเพื่ออัปโหลดรูปถ่าย</div>
                                <div style="font-size: 11.5px; color: #64748b;">รองรับไฟล์ JPG, PNG สูงสุด 3MB</div>
                            </div>
                            <div id="imagePreviewBox" style="display: none; align-items: center; justify-content: center; gap: 12px; margin-top: 8px;">
                                <img id="imagePreviewImg" src="" alt="ตัวอย่างรูปถ่าย" style="max-height: 100px; border-radius: 8px; border: 1px solid #cbd5e1; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
                    ...
                    <div class="form-group" style="flex: 1.5;">
                        ...
                        <textarea id="notes" name="notes" class="form-control" rows="4" placeholder="บันทึกข้อมูลเพิ่มเติม เช่น ประวัติการโยกย้าย, รหัสพัสดุเดิม, อุปกรณ์ต่อพ่วงที่มาด้วยกัน...">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="background: #ffffff; border: 1px solid var(--border); border-radius: 14px; padding: 18px 24px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 14px; box-shadow: 0 4px 16px rgba(0,0,0,0.04);">
            <a href="{{ route('assets.index') }}" class="btn btn-secondary" style="display: inline-flex; align-items: center; gap: 6px;">
                <i class="bi bi-x-circle"></i>
                <span>ยกเลิก</span>
            </a>

            <div style="display: flex; align-items: center; gap: 10px;">
                <button type="reset" class="btn btn-light border" style="display: inline-flex; align-items: center; gap: 6px;" onclick="handleReset()">
                    <i class="bi bi-arrow-counterclockwise"></i> ล้างฟอร์ม
                </button>
...
{{-- Reusable CSV Import Modal for Admin --}}
...
<script>
    ...
    function handlePurchaseDateChange(val) {
        ...
        if (val && budgetYearInput && (!budgetYearInput.value || budgetYearInput.value === '2568')) {
            ...
            // Thai Fiscal Year: Oct (month 10) starts new budget year
    ...
    function handleReset() {
        setTimeout(() => {
            document.querySelectorAll('.type-pill-btn').forEach(btn => btn.classList.remove('active'));
            document.querySelectorAll('.status-radio-card').forEach(c => c.classList.remove('active-status'));
            const activeCard = document.querySelector('.status-radio-card input[value="active"]')?.parentElement;
            if (activeCard) activeCard.classList.add('active-status');
            removeAssetImage();

            // Reset ICT Standard box
            const ictSelect = document.getElementById('ict_standard_select');
            if (ictSelect) ictSelect.value = '';
    ...
    function handleIctStandardSelect(code) {
        ...
        // 3. Recommend Standard Price if price is empty
        const priceInput = document.getElementById('price');
        if (priceInput && (!priceInput.value || priceInput.getAttribute('data-auto-filled') === 'true')) {
            priceInput.value = parseFloat(item.standard_price).toFixed(2);
            priceInput.setAttribute('data-auto-filled', 'true');
        }

        // 4. Fill Recommended Specs (if available)
        if (item.specs) {
            const setVal = (id, val) => {
            ...
            setVal('cpu_model', item.specs.cpu_model);
            setVal('cpu_speed', item.specs.cpu_speed);
            setVal('ram_capacity', item.specs.ram_capacity);
            setVal('ram_type', item.specs.ram_type);
            setVal('ram_bus', item.specs.ram_bus);
            setVal('ram_slots', item.specs.ram_slots);
            setVal('storage_type', item.specs.storage_type);
            setVal('storage_capacity', item.specs.storage_capacity);
            setVal('storage_second', item.specs.storage_second);
            setVal('os_name', item.specs.os_name);
            setVal('os_license', item.specs.os_license);
            setVal('gpu_model', item.specs.gpu_model);
            setVal('monitor_size', item.specs.monitor_size);
